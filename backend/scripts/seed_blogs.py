...
from models import blog, user, project, newsletter  # Import all models to register relationships
...
async def seed_blogs():
    ...
    async with AsyncSessionLocal() as db:
        try:
            # 1. 기존 블로그 삭제
            print("🗑️  기존 블로그 데이터 삭제 중...")
            await db.execute(delete(Blog))
            await db.commit()
            print("✅ 기존 데이터 삭제 완료")

            # 2. Admin 사용자 확인 또는 생성
            ...
            blogs = [
                {
                    ...
                    "content": """# FastAPI + Next.js로 풀스택 AI 블로그 플랫폼 개발하기
...
""",
                    "author_id": admin_user.id,
                    "status": "published",
                    "tags": ["TypeScript", "React", "React Query", "타입안전성"]
                }
            ]
            ...
            for blog_data in blogs:
                blog = Blog(**blog_data)
                db.add(blog)
                print(f"  ✅ '{blog.title}' 생성")

            await db.commit()
            print(f"\n🎉 총 {len(blogs)}개의 블로그 포스트가 생성되었습니다!")
